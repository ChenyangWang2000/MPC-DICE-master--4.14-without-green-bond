...
for i=1:horizon
    ...
    Net_Output(i) = Damages_Fraction(i)*(1-taot)*(1 - Abatement_Fraction(i))*...%revised
        Gross_Economic_Output(i);    
    E_Industrial(i) = sigma(i)*(1-miu(i))*Gross_Economic_Output(i);
    Per_Cap_Consumption(i) = 1000*(1+rb) * Net_Output(i) / L(i);
...
clear_list = {'index_w','index_x','lambda','lambda_G','res','SequenceOCP',...
